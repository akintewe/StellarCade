/**
 * Tests for src/utils/v1/errorMapper.ts
 *
...
import { ContractName, ErrorDomain, ErrorSeverity } from "@/types/errors";
...
describe("mapRpcError", () => {
  ...
  it("maps Soroban simulation error object to RPC_SIMULATION_FAILED", () => {
    ...
    expect(result.severity).toBe(ErrorSeverity.TERMINAL);
  });
  ...
  it("attaches caller-provided context", () => {
    const result = mapRpcError(new Error("Failed to fetch"), {
      requestId: "abc123",
    });
...
describe("mapApiError", () => {
  ...
  it("maps status 401 to API_UNAUTHORIZED (user_actionable)", () => {
    ...
    const result = mapApiError({
      error: { message: "Invalid token", code: "AUTH_FAIL", status: 401 },
    });
...
describe("mapWalletError", () => {
  ...
  it("falls back to WALLET_UNKNOWN for unrecognized messages", () => {
    const result = mapWalletError(
      new Error("Something unexpected happened in the wallet"),
    );
...
describe("mapContractError — prize_pool", () => {
  ...
  it.each([
    [1, "CONTRACT_ALREADY_INITIALIZED", ErrorSeverity.TERMINAL],
    [2, "CONTRACT_NOT_INITIALIZED", ErrorSeverity.TERMINAL],
    ...
    [6, "CONTRACT_GAME_ALREADY_RESERVED", ErrorSeverity.TERMINAL],
    [7, "CONTRACT_RESERVATION_NOT_FOUND", ErrorSeverity.TERMINAL],
    [8, "CONTRACT_PAYOUT_EXCEEDS_RESERVATION", ErrorSeverity.TERMINAL],
    [9, "CONTRACT_OVERFLOW", ErrorSeverity.TERMINAL],
...
describe("mapContractError — random_generator", () => {
  ...
  it("shared slots (1-3) map the same as other contracts", () => {
    expect(mapContractError({ code: 1 }, contract).code).toBe(
      "CONTRACT_ALREADY_INITIALIZED",
    );
    expect(mapContractError({ code: 2 }, contract).code).toBe(
      "CONTRACT_NOT_INITIALIZED",
    );
    expect(mapContractError({ code: 3 }, contract).code).toBe(
...
describe("toAppError", () => {
  ...
  it("attaches context when provided", () => {
    const result = toAppError(new Error("Failed to fetch"), ErrorDomain.RPC, {
      gameId: "g1",
    });
...
describe("validatePreconditions", () => {
  ...
  it("returns CONTRACT_NOT_INITIALIZED for blank contract address", () => {
    ...
    expect(err?.severity).toBe(ErrorSeverity.TERMINAL);
  });
...
describe("formatForLog", () => {
  ...
  it("includes context when present", () => {
    const err = toAppError(new TypeError("Failed to fetch"), ErrorDomain.RPC, {
      reqId: "99",
    });
    const line = formatForLog(err);
